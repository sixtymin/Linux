...
![两种试图](./ELF-Formats-Two-Vision.png)

**链接视图**

链接视图其实是由节表进行组织。在前面介绍目标文件时节表已经有了详细解析，这里列举出内容，然后对于不同点进行解释。
...
第二个节区为`.note.ABI-tag`，它用于定义一个注释，用于标识该ELF为什么操作系统ABI生成。
...
第四个节为`.dynsym`，动态链接符号节，它的格式和`.symtab`相同，并且它其中所包含的符号在`.symtab`中都有包含，即相当与`.symtab`的一个子集。不同地方是这个节会被加载到内存，用于动态链接，而`.symtab`不会加载到内存，并且使用`strip`命令可以将`.symtab`节删除掉。
...
所以这个地址上最终会被动态链接时填上`__gmon_start__`函数的地址，这个函数是系统开始记录`profiling`信息时需要调用的函数。
...
调用后则跳转到`puts@plt`这段代码上执行，它马上进行间接跳转，跳转的目标根据虚拟地址可知位于`.got.plt`中，值为`0x08048376`，这个地址值就是`jmp`指令后的指令`push $0x8`，压栈后跳转到`0x08048350`，即`printf@plt-0x10`，其实就是上面解释过的`.PLT0`，这里会调用到`__dl_runtime_resolve`函数，它有两个参数，一个是前面`push $0x08`，第二个就是`pushl 0x0804a004`，该函数根据`link_map`解析动态库中的`puts`函数的地址，并设置`.got.plt`中的对应地址值，由`0x08048370`改为真实的`puts`函数的地址。以后再调用到时，则直接就跳转到`puts`函数上执行了。
...
其实模块的映射过程中，首先肯定要根据ELF文件头找到程序段表，然后根据程序段表内容将需要映射到内存的段映射进内存；然后根据模块静态链接还是动态链接，如果动态链接则需要首先加载动态链接器（根据`INTERP`内容）；在解析`DYNAMIC`段，它其实对应了`.dynamic`节，如果有需要加载的`.so`文件则依次加载，根据重定位表和动态符号导入进行重定位和符号解析；最后才会进入到可执行文件的模块中起点开始执行（这只是简单说明，其中略过很多细节）。
