...
setup_idt:
	...
	lea idt,%edi
	mov $256,%ecx
rp_sidt:               /*将IDT表都设置为默认处理函数 ignore_int*/
	movl %eax,(%edi)
	movl %edx,4(%edi)
	addl $8,%edi
	dec %ecx
...
after_page_tables:
	...
	jmp setup_paging
L6:
	jmp L6			# 认为main函数不会返回，一旦返回就是出现错误，这里进入死循环
 ...
 * 现在最多支持 16Mb内存，如果要支持更大内存，需要修改源码中的限制
 * 
 * 这里用5页内存，其中一个页目录 和 四个页表 用于映射16M 物理内存
 * 这里是为第一个进程创建的页表映射，其后创建的进程则将这些信息
 * 直接复制到 自己的页目录项中（所有进程共享一个页表）
 */
.align 2
setup_paging:
